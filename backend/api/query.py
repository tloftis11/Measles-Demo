...
def _build_system_prompt(state: str) -> str:
    ...
    schema = (
        ...
        f"                 scoring_source ['hermesboost'|'internal_engine'],\n"
        f"                 probability_pct 0-100, predicted_magnitude)\n"
        ...
        + (
            "For Texas (scoring_source = 'hermesboost'): composite_score is a trained "
            "ML model's risk_score rescaled to a 0-100 percentile rank among currently-"
            "scored TX counties (100 = highest relative risk); coverage_score and "
            "surveillance_score are NULL (no 3-layer breakdown for this methodology); "
            "network_score holds district MMR variance; probability_pct is the "
            "likelihood of any confirmed case; predicted_magnitude is the predicted "
            "case count if one occurs.\n"
            "For Idaho/Pennsylvania (scoring_source = 'internal_engine'): "
            "composite_score = 0.40×coverage + 0.35×surveillance + 0.25×network, the "
            "hand-weighted formula below; probability_pct and predicted_magnitude are NULL.\n"
            if m["abbr"] == "TX"
            else "Composite score = 0.40×coverage + 0.35×surveillance + 0.25×network "
            "(scoring_source = 'internal_engine' for this state; the hermesboost "
            "methodology only applies to Texas).\n"
        )
